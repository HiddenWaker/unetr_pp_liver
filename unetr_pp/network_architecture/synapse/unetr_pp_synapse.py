...
class UNETR_PP(SegmentationNetwork):
    ...
    def __init__(
        ...
        self.unetr_pp_encoder_gzx = UnetrPPEncoder(dims=dims, depths=depths, num_heads=num_heads) # adding 10/3
        self.unetr_pp_encoder_gzy = UnetrPPEncoder(dims=dims, depths=depths, num_heads=num_heads) # adding 10/5
    ...
    def hessian_matrix(self, x_in):
        '''
        Make hessian matrix
        -> second-derivative function in matrix
        '''
        # print(x_in.shape) # torch.Size([1, 1, 64, 128, 128])
        with torch.no_grad():
            gx, gy, gz = torch.gradient(x_in[0][0]) # 1차 미분
            gzx, gzy, _ = torch.gradient(gz) # 2차 미분
            gzx = gzx.unsqueeze(0).unsqueeze(0)
            gzy = gzy.unsqueeze(0).unsqueeze(0)
    ...
    def forward(self, x_in):
        ...
        # 10/3 add: hessian encoder
        
        gzx_in, gzy_in = self.hessian_matrix(x_in)
        gzx_output, hidden_states_gzx = self.unetr_pp_encoder_gzx(gzx_in)
        gzy_output, hidden_states_gzy = self.unetr_pp_encoder_gzy(gzy_in)
        
        # 10/3 adding
        # (Four) hessian encoders
        enc1_gzx = hidden_states_gzx[0]
        enc2_gzx = hidden_states_gzx[1]
        enc3_gzx = hidden_states_gzx[2]
        enc4_gzx = hidden_states_gzx[3]
        
        enc1_gzy = hidden_states_gzy[0]
        enc2_gzy = hidden_states_gzy[1]
        enc3_gzy = hidden_states_gzy[2]
        enc4_gzy = hidden_states_gzy[3]
        
        # 10/3 adding
        # Add hessian encoder and encoder
        ## 추후에 곱셈 가즈아
        enc1 = enc1 + enc1_gzx + enc1_gzy
        enc2 = enc2 + enc2_gzx + enc2_gzy
        enc3 = enc3 + enc3_gzx + enc3_gzy
        enc4 = enc4 + enc4_gzx + enc4_gzy
