...
from unetr_pp.training.loss_functions.crossentropy import RobustCrossEntropyLoss, HausdorffDistance, FocalLoss
from unetr_pp.training.loss_functions.focal_loss import FocalLoss
...
class SoftDiceLoss(nn.Module):
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        if not self.do_bg: # background No
            if self.batch_dice:
                dc = dc[1:]
            else:
                dc = dc[:, 1:]
...
class DC_and_CE_and_HD_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,
                 weight_hd=1, log_dice=False, ignore_label=None, focal=False):
        """
    ...
    def forward(self, net_output, target):
        ...
        #print(net_output.shape) # torch.Size([8, 2, 64, 128, 128])
        ...
        if False:
            ...
            print("HD loss:", hd_loss) # check first
            print("#######")
...
class DC_and_FCE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_fce=1, weight_dice=1,
                 log_dice=False, ignore_label=None):
        """
        ...
        :param weight_dice:
        """
        super(DC_and_FCE_loss, self).__init__()
        if ignore_label is not None:
            assert not square_dice, 'not implemented'
            ce_kwargs['reduction'] = 'none'
        self.log_dice = log_dice
        self.weight_dice = weight_dice # 1
        self.weight_fce = weight_fce # 1
        self.aggregate = aggregate
        self.fce = FocalLoss(**ce_kwargs) # use focal loss
    ...
    def forward(self, net_output, target):
        ...
        if self.log_dice:
            dc_loss = -torch.log(-dc_loss)
        #print(dc_loss) # -0.4702

        fce_loss = self.fce(net_output, target[:, 0].long()) if self.weight_fce != 0 else 0
        if self.ignore_label is not None:
            fce_loss *= mask[:, 0]
            fce_loss = fce_loss.sum() / mask.sum()
        print(fce_loss)

        if self.aggregate == "sum":
            result = self.weight_fce * fce_loss + self.weight_dice * dc_loss
        else:
            raise NotImplementedError("nah son") # reserved for other stuff (later)
        return result  
