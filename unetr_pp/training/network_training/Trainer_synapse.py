...
from unetr_pp.training.loss_functions.dice_loss import DC_and_CE_loss, DC_and_FCE_loss, DC_and_CE_and_HD_loss
#from unetr_pp.training.loss_functions.focal_loss import AsymmetricUnifiedFocalLoss
...
class Trainer_synapse(NetworkTrainer_synapse):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.loss = DC_and_CE_and_HD_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {})
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
        ...
        inference_times = []
        test_path = '../DATASET_Synapse/unetr_pp_raw/unetr_pp_raw_data/Task02_Synapse/Task002_Synapse/unetr_pp_Data_plans_v2.1_stage1/test/'
        #folder_name = os.listdir(test_path)
        folder_name = [folder for folder in os.listdir(test_path) if '.npy' not in folder]
        print(folder_name)
        for k in folder_name:
            ...
            if True: #overwrite or (not isfile(join(output_folder, fname + ".nii.gz"))) or \
                    #(save_softmax and not isfile(join(output_folder, fname + ".npz"))):
                start = time.time()
                ct_npy = fname+'.npy'
                data = np.load(test_path + ct_npy)
                print(k, data.shape)
                data[-1][data[-1] == -1] = 0
                ...
                print(softmax_pred.shape) # (2, 245, 471, 471)
                # // Remember // Recover origin spacing? function
                results.append(export_pool.starmap_async(save_segmentation_nifti_from_softmax,
                                                         ((softmax_pred, join(output_folder, fname + ".nii.gz"),
                                                           None, interpolation_order, self.regions_class_order,
                                                           None, None,
