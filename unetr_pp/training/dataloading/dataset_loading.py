...
def get_case_identifiers(folder):
    # npz -> mha -> nii.gz
    case_identifiers = [i[:-7] for i in os.listdir(folder) if i.endswith(".nii.gz") and (i.find("segFromPrevStage") == -1)]
...
#### mha to npy
#../DATASET_Synapse/unetr_pp_raw/unetr_pp_raw_data/Task02_Synapse\Task002_Synapse\unetr_pp_Data_plans_v2.1_stage1\img0001.mha
#../DATASET_Synapse/unetr_pp_raw/unetr_pp_raw_data/Task02_Synapse\Task002_Synapse\unetr_pp_Data_plans_v2.1_stage1\img0002.mha
#../DATASET_Synapse/unetr_pp_raw/unetr_pp_raw_data/Task02_Synapse\Task002_Synapse\unetr_pp_Data_plans_v2.1_stage1\img0003.mha
#../DATASET_Synapse/unetr_pp_raw/unetr_pp_raw_data/Task02_Synapse\Task002_Synapse\unetr_pp_Data_plans_v2.1_stage1\img0004.mha
#../DATASET_Synapse/unetr_pp_raw/unetr_pp_raw_data/Task02_Synapse\Task002_Synapse\unetr_pp_Data_plans_v2.1_stage1\img0005.mha
def convert_to_npy(args):
    ...
    if not isfile(nii_files[:-6] + "npy"):
        ...
        # ToDo: 9/16: preprocessing 적용하기
        # 9/17: preprocessing method 변경하기 
        # a = nib.load(nii_files).get_fdata() # nii load
        # UNETR++ Preprocessing
        # First, (axial view, , )가 되도록 shape chages
        # (논문에 기재된 순서로 ㄱㄱ)
        # 1. normalize [0,1]
...
def preprocessing(input_nii, gt):
    # change voxel spacing -> (1.0, 1.0, 1.0)
    itk_image = resample_img(input_nii, out_spacing=[1.0, 1.0, 1.0], is_label=False)
    itk_gt = resample_img(gt, out_spacing=[1.0, 1.0, 1.0], is_label=True)
    
    itk_image_np = sitk.GetArrayFromImage(itk_image) #Converting sitk_metadata to image Array
    itk_image_gt = sitk.GetArrayFromImage(itk_gt) #Converting sitk_metadata to image Array
    #print("path:",gt)
    print(np.unique(itk_image_gt))
    #print(np.where(itk_image_gt==4.0))
    
    # normalize apply
    itk_image_np = (itk_image_np-np.min(itk_image_np))/(np.max(itk_image_np)-np.min(itk_image_np))
    
    return itk_image_np, itk_image_gt
...
def unpack_dataset(folder, threads=default_num_threads, key="data"):
    ...
    """
    p = Pool(threads)
    nii_files = subfiles(folder, True, None, ".nii.gz", True) # npz->mha
...
def load_dataset(folder, num_cases_properties_loading_threshold=1000):
    ...
    dataset = OrderedDict()
    #print(case_identifiers)
    for c in case_identifiers:
        dataset[c] = OrderedDict()
        dataset[c]['data_file'] = join(folder, "%s.nii.gz" % c) # npz -> mha -> nii.gz
...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            if isfile(self._data[i]['data_file'][:-7] + ".npy"):
                case_all_data = np.load(self._data[i]['data_file'][:-7] + ".npy", self.memmap_mode)
            else:
                case_all_data = np.load(self._data[i]['data_file'])['data']
            #print(case_all_data.shape) # (2, 512, 512, 174)
