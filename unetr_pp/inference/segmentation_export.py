...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    if properties_dict != None:
        ...
        # Just Whole liver segmentation. --> Will appy couinaud segmentation
        # print(segmentation_softmax.shape)
        print("############## Saving... ###################")
        print(segmentation_softmax)
        segmentation_softmax = segmentation_softmax.argmax(0)
        print(segmentation_softmax)
        sitk.WriteImage(segmentation_softmax, out_fname) # save
